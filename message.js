...
const statusKehadiran = document.querySelector(".status-kehadiran");
...
function fetchMessage() {
  ...
  fetch(scriptURLGet)
    .then((resp) => resp.json())
    .then((data) => {
      statusKehadiran.innerHTML = `<small class="status-kehadiran">Hadir : ${data[0].jumlahhadir} | Tidak Hadir : ${data[0].jumlahtidakhadir} </small>`;
      maker(data);
    });
}
...
function maker(data) {
  ...
  data.reverse().forEach((message) => {
    if (
      message.nama.trim().length !== 0 &&
      message.pesan.trim().length !== 0 &&
      message.kehadiran.trim().length !== 0
    ) {
